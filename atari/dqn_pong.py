...
class Estimator(object):
    def __init__(self, sess, summaries_dir='/home/hzyuan/DRL/DQN', batch_size=32, act_n=6, name='q_estimator'):
        self.sess = sess
    ...
    def _build_model(self):
        with tf.variable_scope(self.name) as scope:
            ...
            matrix = tf.one_hot(tf.reshape(self.act, [-1]), self.act_n, 1.0, 0.0, dtype=tf.float32)
            print(self.predictions)
            print(matrix)
            matrix = self.predictions * 10
            matrix
            ...
            self.outer_sums = tf.summary.merge(
                [tf.summary.scalar('epsilon', self.ep), tf.summary.scalar('reward', self.reward)])
...
def DQN(env, sess, q_estimator, processer, episodes=500, discount_factor=0.9, max_memory=5000, train_steps=1,
        batch_size=32, act_n=6, update_steps=100):
    memory = []
    count = 0
    done = False
